...
#define MAX_STRING_LENGTH 1000   // 为了安全起见限制允许存入队列的最大字符串长度1000

// 下面队列存入的字符串每次都是动态内存分配的,无需提前占用过多内存空间，但用完后需要注意释放内存
typedef struct {
    char** queue;   // 存储数据指针的数组（个数未确定）
    int front;      // 队列头
    int rear;       // 队列尾
    int count;      // 队列存储数据个数
    int cap;        // 队列的容量
...
void initializeCircularQueue(CircularQueue* queue, int cap) {
    // 分配cap个内存空间用来存储队列中数据指针的存储空间（返回的是申请的内存空间的指针）
    queue->queue = (char**)malloc(cap * sizeof(char*));
...
int main() {
    ...
    CircularQueue queue;
    // 队列最大数据个数MAX_SIZE
    initializeCircularQueue(&queue, MAX_SIZE);
